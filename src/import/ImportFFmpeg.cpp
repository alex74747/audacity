   ...
   const FilePath &filename, AudacityProject*)
{
   ...
   wxString extension = wxString{filename.GET()}.AfterLast(L'.');
   if (SupportsExtension(extension))
   {
      ...
      gPrefs->Read(L"/NewImportingSession", &newsession);
      if (!FFmpegLibsInst()->ValidLibsLoaded())
      {
         ...
         {
            gPrefs->Write(L"/NewImportingSession", false);
            gPrefs->Flush();
...
bool FFmpegImportFileHandle::Init()
{
   ...
   if (err < 0)
   {
      wxLogError(L"FFmpeg : av_open_input_file() failed for file %s", mName);
      return false;
   }
   ...
   if (err < 0)
   {
      wxLogError(L"FFmpeg: avformat_find_stream_info() failed for file %s",mName);
      return false;
   }
...
bool FFmpegImportFileHandle::InitCodecs()
{
   ...
   for (unsigned int i = 0; i < mFormatContext->nb_streams; i++)
   {
      if (mFormatContext->streams[i]->codec->codec_type == AVMEDIA_TYPE_AUDIO)
      {
         ...
         if (codec == NULL)
         {
            wxLogError(L"FFmpeg : avcodec_find_decoder() failed. Index[%02d], Codec[%02x - %s]",i,id,name);
         ...
         {
            wxLogError(L"FFmpeg : Codec type mismatch, skipping. Index[%02d], Codec[%02x - %s]",i,id,name);
         ...
         {
            wxLogError(L"FFmpeg : avcodec_open() failed. Index[%02d], Codec[%02x - %s]",i,id,name);
         ...
         if (sc->m_codecCtx->bit_rate > 0)
            bitrate.Printf(L"%d",(int)sc->m_codecCtx->bit_rate);
         else
            bitrate.Printf(L"?");
              ...
              Tags *tags)
{
   ...
   for (auto &stream : mChannels)
   {
      ...
      if (sc->m_stream->start_time != int64_t(AV_NOPTS_VALUE) && sc->m_stream->start_time > 0)
      {
         ...
         wxLogDebug(L"Stream %d start_time = %lld, that would be %f milliseconds.", s, (long long) sc->m_stream->start_time, double(sc->m_stream->start_time)/AV_TIME_BASE*1000);
      }
      if (stream_delay > 0)
      {
         int c = -1;
      ...
      for (size_t chn = 0; (int)chn < sc->m_stream->codec->channels; chn++)
      {
         if (chn < nChannels)
         {
            switch (sc->m_samplefmt)
            {
               ...
               default:
                  wxLogError(L"Stream %d has unrecognized sample format %d.", streamid, sc->m_samplefmt);
                  return ProgressResult::Success;
               break;
            }
         }
