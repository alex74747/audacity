      ...
      {   // Show the dialog and bail if the user cancels
         ...
         for( curve = 0; curve < numCurves; curve++ )
         {
            ...
            if( name.IsSameAs( mEditCurves[ curve ].Name )) // case sensitive
            {
               ...
               int answer = mEffect->Effect::MessageBox( wxString::Format(_("Overwrite existing curve '%s'?"), name),
...
void EditCurvesDialog::OnDelete(wxCommandEvent & WXUNUSED(event))
{
   ...
   while(item >= 0)
   {
      ...
      {
         Effect::MessageBox(_("You cannot delete the 'unnamed' curve."),
      ...
      else
      {
         // Create the prompt
         wxString quest;
         quest = wxString::Format(_("Delete '%s'?")), mEditCurves[ item-deleted ].Name);

         // Ask for confirmation before removal
         int ans = Effect::MessageBox( quest, _("Confirm Deletion"), wxYES_NO | wxCENTRE, this );
         if( ans == wxYES )
   ...
   wxString quest;
   if( count > 1 )
      quest.Printf(_("Delete %d items?"), count);
   else
      if( count == 1 )
