...
void EqualizationPanel::OnPaint(wxPaintEvent &  WXUNUSED(event))
{
   ...
   if( mParent->mFaderOrDraw[0]->GetValue() )
      mEnvelope->DrawPoints(memDC, mEnvRect, ZoomInfo(0.0, 1.0, mEnvRect.width-1), false, dBMin, dBMax);
...
void EqualizationPanel::OnMouseEvent(wxMouseEvent & event)
{
   ...
   if (mEnvelope->MouseEvent(event, mEnvRect, ZoomInfo(0.0, 1.0, mEnvRect.width), false,
      dBMin, dBMax))
   {
