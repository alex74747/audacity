...
void EqualizationPanel::OnPaint(wxPaintEvent &  WXUNUSED(event))
{
   ...
   if( mEffect->mDraw->GetValue() )
      mEffect->mEnvelope->DrawPoints(memDC, mEnvRect, ZoomInfo(0.0, 1.0, mEnvRect.width-1), false, mEffect->mdBMin, mEffect->mdBMax);
...
void EqualizationPanel::OnMouseEvent(wxMouseEvent & event)
{
   ...
   if (mEffect->mEnvelope->MouseEvent(event, mEnvRect, ZoomInfo(0.0, 1.0, mEnvRect.width - 1), false,
