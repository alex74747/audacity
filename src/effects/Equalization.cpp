...
static auto FilterLength = Parameter<int>(
                           L"FilterLength",        8191,    21,      8191,    0      );
static auto CurveName = Parameter<const wxChar*>(
                           L"CurveName",           L"unnamed", L"", L"", L"");
static auto InterpLin = Parameter<bool>(
                           L"InterpolateLin",      false,   false,   true,    false  );
static auto InterpMeth = EnumParameter(
                           L"InterpolationMethod", 0,       0,       0,       0, kInterpStrings, nInterpolations      );
static auto DrawMode = Parameter<bool>(
                           L"",                   true,    false,   true,    false  );
static auto DrawGrid = Parameter<bool>(
                           L"",                   true,    false,   true,    false  );
static auto dBMin = Parameter<float>(
                           L"",                   -30.0,   -120.0,  -10.0,   0      );
static auto dBMax = Parameter<float>(
                           L"",                   30.0,    0.0,     60.0,    0      );
   ...
   , mFilterFuncI{ windowSize }
{
   ...
   GetConfig(GetDefinition(), PluginSettings::Private,
      GetCurrentSettingsGroup(), L"dBMin", mdBMin, dBMin.def);
   GetConfig(GetDefinition(), PluginSettings::Private,
      GetCurrentSettingsGroup(), L"dBMax", mdBMax, dBMax.def);
   GetConfig(GetDefinition(), PluginSettings::Private,
      GetCurrentSettingsGroup(), L"DrawMode", mDrawMode, DrawMode.def);
   GetConfig(GetDefinition(), PluginSettings::Private,
      GetCurrentSettingsGroup(), L"DrawGrid", mDrawGrid, DrawGrid.def);
...
bool EffectEqualization::ValidateUI()
{
   ...
   SetConfig(GetDefinition(), PluginSettings::Private,
      GetCurrentSettingsGroup(), L"dBMin", mdBMin);
   SetConfig(GetDefinition(), PluginSettings::Private,
      GetCurrentSettingsGroup(), L"dBMax", mdBMax);
   SetConfig(GetDefinition(), PluginSettings::Private,
      GetCurrentSettingsGroup(), L"DrawMode", mDrawMode);
   SetConfig(GetDefinition(), PluginSettings::Private,
      GetCurrentSettingsGroup(), L"DrawGrid", mDrawGrid);
      ...
      S.Prop(1).StartMultiColumn(7, wxALIGN_CENTER_HORIZONTAL);
      {
         ...
         if( mOptions == kEqLegacy ){
            S.StartHorizontalLay(wxEXPAND, 0);
            {
               ...
               {
                  S
                     .AddPrompt(XXO("Length of &Filter:"));
               }
               S.EndHorizontalLay();
...
// Just going via .cfg files now.
   ...
   (void)append;
   mCurves.clear();
   mCurves.emplace_back( L"unnamed" );   // we still need a default curve to use
#else
   ...
   if (updateAll) {
      ...
      for (int curveCount = 0; curveCount < numUserCurves; curveCount++) {
         ...
         for (int defCurveCount = 0; defCurveCount < numDefaultCurves; defCurveCount++) {
            if (tempCurve == mCurves[defCurveCount]) {
               isCustom = false;
               break;
            }
         }
      ...
      for (int defCurveCount = 0; defCurveCount < numDefaultCurves; defCurveCount++) {
         ...
         for (int userCurveCount = 0; userCurveCount < numUserCurves; userCurveCount++) {
            if (userCurves[userCurveCount] == defaultCurves[defCurveCount]) {
               isUserCurve = true;
      ...
      for (int userCurveCount = 0; userCurveCount < numUserCurves; userCurveCount++) {
         ...
         for (int defCurveCount = 0; defCurveCount < numDefaultCurves; defCurveCount++) {
            if (tempCurve == defaultCurves[defCurveCount]) {
               isDefaultCurve = true;
               break;
            }
         }
...
void EditCurvesDialog::PopulateList(int position)
{
   ...
   for (unsigned int i = 0; i < mEditCurves.size(); i++)
      mList->InsertItem(i, mEditCurves[i]);
...
void EditCurvesDialog::OnExport()
{
   ...
   while(item >= 0)
   {
      ...
      {
         exportCurves.emplace_back(mEditCurves[item]);
         i++;
      }
      else
         mEffect->Effect::MessageBox(
