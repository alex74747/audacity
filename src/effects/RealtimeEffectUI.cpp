   ...
   mList(list)
{
   ...
   Create(&GetProjectFrame(manager->GetProject()), wxID_ANY, title,
...
{
   auto latency = mManager->GetLatency();
   if (latency != mLastLatency)
   {
      using Ms = std::chrono::duration<float, std::milli>;
      float ms{ std::chrono::duration_cast<Ms>(latency).count() };
      mLatency->SetLabel(wxString::Format(_("Latency: %.3f ms"), ms));
      mLatency->Refresh();
...
void RealtimeEffectUI::Selector::PopulateOrExchange(ShuttleGui &S)
{
   ...
   for (const auto &plug : pm.PluginsOfType(EffectTypeProcess)) {
