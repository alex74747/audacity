...
#endif
{
   ...
   theProject->Bind(
      EVT_SYNC_LOCK_CHANGE, &TrackPanel::OnSyncLockChange, this);
   theProject->Bind(
...
void TrackPanel::OnIdle(wxIdleEvent& event)
{
   ...
   if (IsShownOnScreen())
   {
      mTimer.Start(std::chrono::milliseconds{TrackPanelTimerInterval}.count(), FALSE);
...
void TrackPanel::OnSyncLockChange( SyncLockChangeEvent &event )
{
   event.Skip();
   Refresh(false);
}
