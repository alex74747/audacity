   ...
   for (const auto &clip : mClips)
   {
      ...
      else if (!clip->BeforeClip(t1) && !clip->AfterClip(t0))
      {
         ...
         if (addCutLines)
         {
            ...
            clipsToDelete.push_back( clip.get() );
            auto newClip = std::make_shared<WaveClip>( *clip, mpFactory, true );
...
void WaveTrack::InsertSilence(double t, double len)
{
   ...
   if (mClips.empty())
   {
      ...
      auto clip = std::make_shared<WaveClip>(mpFactory, mFormat, mRate, this->GetWaveColorIndex());
...
{
   mClips.push_back(std::make_shared<WaveClip>(mpFactory, mFormat, mRate, GetWaveColorIndex()));
