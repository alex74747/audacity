...
WaveTrack::WaveTrack(const std::shared_ptr<DirManager> &projDirManager, sampleFormat format, double rate) :
   PlayableTrack()
   , mpData{ std::make_shared<WaveTrackData>( projDirManager, format, rate ) }
{
   ...
   )
{
   ...
   auto &clips = GetData()->GetClips();
   for (const auto &clip : orig.GetData()->GetClips())
      clips.push_back
         ( std::make_unique<WaveClip>(
            *clip, GetData()->GetDirManager(), true ) );
}
...
void WaveTrack::Merge(const Track &orig)
{
   orig.TypeSwitch( [&](const WaveTrack *pwt) {
      ...
      auto waveTrackData = this->GetData();
      auto waveTrackDataOther = wt.GetData();
      waveTrackData->SetGain( waveTrackDataOther->GetGain() );
      waveTrackData->SetPan( waveTrackDataOther->GetPan() );
...
auto WaveTrackData::GetChannelIgnoringPan() const -> ChannelType {
   return mChannel;
}

auto WaveTrackData::GetChannel() const -> ChannelType
{
...
void WaveTrackData::SetPanFromChannelType()
{ 
...
{
   const double rate = GetData()->GetRate();
...
void WaveTrackData::InitializeRate( double newRate )
{
   mRate = lrint(newRate);
}

void WaveTrackData::SetRate(double newRate)
{
   wxASSERT( newRate > 0 );
...
void WaveTrackData::SetGain(float newGain)
{
   if (mGain != newGain) {
      mGain = newGain;
    //  Notify();
...
void WaveTrackData::SetPan(float newPan)
{
   if (newPan > 1.0)
      newPan = 1.0;
   else if (newPan < -1.0)
...
float WaveTrackData::GetChannelGain(int channel) const
{
   float left = 1.0;
   float right = 1.0;
...
void WaveTrackData::ConvertToSampleFormat(sampleFormat format)
// WEAK-GUARANTEE
...
Track::Holder WaveTrack::Copy(double t0, double t1, bool forClipboard) const
{
   ...
   auto dirManager = GetData()->GetDirManager();
   auto result = std::make_shared<WaveTrack>( dirManager );
   ...
   auto waveTrackDataNew = newTrack->GetData();
   auto &newClips = waveTrackDataNew->GetClips();
   auto waveTrackData = this->GetData();
   auto &clips = waveTrackData->GetClips();
   for (const auto &clip : clips)
   {
      if (t0 <= clip->GetStartTime() && t1 >= clip->GetEndTime())
      {
         ...
         newClips.push_back
            (std::make_unique<WaveClip>(*clip, dirManager, ! forClipboard));
      ...
      else if (t1 > clip->GetStartTime() && t0 < clip->GetEndTime())
      {
         ...
         auto newClip = std::make_unique<WaveClip>
            (*clip, dirManager, ! forClipboard, clip_t0, clip_t1);
         ...
         newClips.push_back(std::move(newClip)); // transfer ownership
      }
   }
   ...
   if (forClipboard &&
       newTrack->GetEndTime() + 1.0 / waveTrackDataNew->GetRate() < t1 - t0)
   {
      auto placeholder = std::make_unique<WaveClip>(waveTrackData->GetDirManager(),
            waveTrackDataNew->GetSampleFormat(),
            static_cast<int>(waveTrackDataNew->GetRate()),
            0 /*colourindex*/);
      placeholder->SetIsPlaceholder(true);
      placeholder->InsertSilence(0, (t1 - t0) - newTrack->GetEndTime());
      placeholder->Offset(newTrack->GetEndTime());
      newClips.push_back(std::move(placeholder)); // transfer ownership
   }
...
// But some of its cutline clips may have been destroyed.
{
   ...
   for (const auto &clip : clips) {
      ...
      st = waveTrackData->LongSamplesToTime(
         waveTrackData->TimeToLongSamples(clip->GetStartTime()));
      if (st >= t0 && st <= t1 && !make_iterator_range(splits).contains(st)) {
         splits.push_back(st);
      }

      st = waveTrackData->LongSamplesToTime(
         waveTrackData->TimeToLongSamples(clip->GetEndTime()));
      if (st >= t0 && st <= t1 && !make_iterator_range(splits).contains(st)) {
         splits.push_back(st);
      }
      ...
      for (auto it = cutlines.begin(); it != cutlines.end(); ) {
         WaveClip *cut = it->get();
         double cs = waveTrackData->LongSamplesToTime(
            waveTrackData->TimeToLongSamples(
               clip->GetOffset() + cut->GetOffset()));
   ...
   const auto tolerance = 2.0 / waveTrackData->GetRate();
...
void WaveTrack::AddClip(std::shared_ptr<WaveClip> &&clip)
{
   ...
   //if (CanInsertClip(clip))
      clips.push_back(std::move(clip)); // transfer ownership
}
...
// STRONG-GUARANTEE
{
   ...
   for (const auto &clip : clips)
   {
      ...
      else if (!clip->BeforeClip(t1) && !clip->AfterClip(t0))
      {
         ...
         if (addCutLines)
         {
            ...
            clipsToDelete.push_back( clip.get() );
            auto newClip = std::make_unique<WaveClip>( *clip, dirManager, true );
            ...
            if (split) {
               ...
               if (clip->BeforeClip(t0)) {
                  ...
                  clipsToAdd.push_back
                     ( std::make_unique<WaveClip>( *clip, dirManager, true ) );
   ...
   {
      auto myIt = FindClip(clips, clip);
      if (myIt != clips.end())
         clips.erase(myIt); // deletes the clip!
      else
         wxASSERT(false);
   }
...
void WaveTrack::SyncLockAdjust(double oldT1, double newT1)
{
   if (newT1 > oldT1) {
      ...
      if (IsEmpty(oldT1, oldT1))
      {
         ...
         if (clipsCanMove) {
            auto tmp = Cut (oldT1, GetEndTime() + 1.0/waveTrackData->GetRate());
...
// WEAK-GUARANTEE
{
   ...
   bool bOk = src && src->TypeSwitch< bool >( [&](const WaveTrack *other) {
      ...
      if( insertDuration != 0 && insertDuration < 1.0/waveTrackData->GetRate() )
      ...
      if (editClipCanMove) {
         if (!singleClipMode) {
            ...
            if (!IsEmpty(t0, GetEndTime())) {
               auto tmp = Cut(t0, GetEndTime()+1.0/waveTrackData->GetRate());
            ...
            for (const auto &clip : clips)
            {
               if (clip->GetStartTime() > t0-(1.0/waveTrackData->GetRate()))
      ...
      if (singleClipMode)
      {
         ...
         for (const auto &clip : clips)
         {
            ...
            else
            {
               ...
               if (clip->WithinClip(t0) ||
                   waveTrackData->TimeToLongSamples(t0) == clip->GetStartSample())
               {
                  insideClip = clip.get();
                  break;
               }
      ...
      if (!editClipCanMove && !IsEmpty(t0, t0+insertDuration-1.0/waveTrackData->GetRate()))
         // STRONG-GUARANTEE in case of this path
         // not that it matters.
         throw SimpleMessageBoxException{
            _("There is not enough room available to paste the selection")
      ...
      for (const auto &clip : other->GetData()->GetClips())
      {
         ...
         if (!clip->GetIsPlaceholder())
         {
            auto newClip =
               std::make_unique<WaveClip>(
                  *clip, waveTrackData->GetDirManager(), true );
            newClip->Resample(waveTrackData->GetRate());
            newClip->Offset(t0);
            newClip->MarkChanged();
            clips.push_back(std::move(newClip)); // transfer ownership
         }
      }
      return true;
   } );
...
void WaveTrack::Silence(double t0, double t1)
{
   ...
   auto start = (sampleCount)floor(t0 * rate + 0.5);
   auto len = (sampleCount)floor(t1 * rate + 0.5) - start;
...
// STRONG-GUARANTEE
{
   ...
   auto waveTrackData = GetData();
   auto dirManager = waveTrackData->GetDirManager();
   auto &clips = waveTrackData->GetClips();
   if (clips.empty())
   {
      // Special case if there is no clip yet
      auto clip = std::make_unique<WaveClip>(
         dirManager, waveTrackData->GetSampleFormat(),
         waveTrackData->GetRate(), this->GetWaveColorIndex());
...
// WEAK-GUARANTEE
{
   auto waveTrackData = GetData();
   auto minSamples =
      waveTrackData->TimeToLongSamples( WAVETRACK_MERGE_POINT_TOLERANCE );
               ...
               if( seqStart != -1 )
               {
                  ...
                  if( seqEnd - seqStart + 1 > minSamples )
                  {
                     regions.push_back(Region(
                        seqStart.as_double() / waveTrackData->GetRate()
                                              + clip->GetStartTime(),
                        seqEnd.as_double() / waveTrackData->GetRate()
                                              + clip->GetStartTime()));
                  }
                  seqStart = -1;
               }
...
// WEAK-GUARANTEE
{
   ...
   for (const auto &clip: clips)
   {
      if (clip->GetStartTime() < t1-(1.0/waveTrackData->GetRate()) &&
          clip->GetEndTime()-(1.0/waveTrackData->GetRate()) > t0) {
   ...
   for (const auto &clip : clipsToDelete)
   {
      ...
      if (clip->GetOffset() - t > (1.0 / waveTrackData->GetRate())) {
...
sampleCount WaveTrackData::GetBlockStart(sampleCount s) const
{
   auto &clips = GetClips();
   for (const auto &clip : clips)
   {
      const auto startSample =
         (sampleCount)floor(0.5 + clip->GetStartTime()*GetRate());
...
size_t WaveTrackData::GetBestBlockSize(sampleCount s) const
{
   ...
   {
      auto startSample =
         (sampleCount)floor(clip->GetStartTime()*GetRate() + 0.5);
...
size_t WaveTrackData::GetMaxBlockSize() const
{
   ...
   if (maxblocksize == 0)
   {
      ...
      maxblocksize =
         Sequence{ mDirManager, GetSampleFormat() }
            .GetMaxBlockSize();
   }
      ...
      while(*attrs) {
         ...
         if (!wxStrcmp(attr, wxT("rate")))
         {
            ...
            if (!XMLValueChecker::IsGoodString(strValue) ||
               ...
               return false;
            GetData()->InitializeRate( dblValue );
         }
         ...
         {}
            ...
            this->SetSelected(nValue != 0);
                  ...
                  XMLValueChecker::IsGoodString(strValue) &&
                  Internat::CompatibleToDouble(strValue, &dblValue))
            waveTrackData->SetGain( dblValue );
         else if (!wxStrcmp(attr, wxT("pan")) &&
                  XMLValueChecker::IsGoodString(strValue) &&
                  Internat::CompatibleToDouble(strValue, &dblValue) &&
                  (dblValue >= -1.0) && (dblValue <= 1.0))
            waveTrackData->SetPan( dblValue );
         ...
         else if (!wxStrcmp(attr, wxT("channel")))
         {
            if (!XMLValueChecker::IsGoodInt(strValue) || !strValue.ToLong(&nValue) ||
               ...
               return false;
            waveTrackData->SetChannel(
               static_cast<Track::ChannelType>( nValue ) );
         }
...
// may throw
{
   ...
   xmlFile.WriteAttr(wxT("channel"), waveTrackData->GetChannelIgnoringPan());
   ...
   xmlFile.WriteAttr(wxT("rate"), waveTrackData->GetRate());
...
AUDACITY_DLL_API sampleCount WaveTrackData::TimeToLongSamples(double t0) const
{
   return sampleCount( floor(t0 * GetRate() + 0.5) );
}

double WaveTrackData::LongSamplesToTime(sampleCount pos) const
{
...
double WaveTrackData::GetStartTime() const
{
   bool found = false;
   double best = 0.0;
...
double WaveTrackData::GetEndTime() const
{
   bool found = false;
   double best = 0.0;
...
bool WaveTrackData::Get(samplePtr buffer, sampleFormat format,
...
void WaveTrackData::GetEnvelopeValues(double *buffer, size_t bufferLen,
                                  double t0) const
{
   ...
   for (const auto &clip: clips)
   {
      ...
      if ((dClipStartTime < endTime) && (dClipEndTime > startTime))
      {
         ...
         if (rt0 < dClipStartTime)
         {
            ...
            auto nDiff = (sampleCount)floor((dClipStartTime - rt0) * GetRate() + 0.5);
...
WaveClip* WaveTrack::CreateClip()
{
   auto waveTrackData = GetData();
   auto &clips = waveTrackData->GetClips();
   clips.push_back(std::make_unique<WaveClip>(
      waveTrackData->GetDirManager(),
      waveTrackData->GetSampleFormat(), waveTrackData->GetRate(),
      GetWaveColorIndex()));
...
// NOFAIL-GUARANTEE
{
   ...
   else
   {
      ...
      for (auto end = clips.end(); it != end; ++it)
      {
...
{
   int result;
   FindClip(const_cast<WaveTrack*>(this)->GetData()->GetClips(), clip, &result);
   return result;
}
...
// WEAK-GUARANTEE
{
   ...
   for (const auto &c : clips)
   {
      if (c->WithinClip(t))
      {
         t = waveTrackData->LongSamplesToTime(
            waveTrackData->TimeToLongSamples(t)); // put t on a sample
         auto newClip = std::make_unique<WaveClip>(
            *c, waveTrackData->GetDirManager(), true );
         ...
         sampleCount here = llrint(floor(((t - c->GetStartTime()) *
            waveTrackData->GetRate()) + 0.5));
         newClip->Offset(here.as_double()/(double)waveTrackData->GetRate());
...
// STRONG-GUARANTEE
{
   ...
   auto &clips = GetData()->GetClips();
   auto pEnd = clips.end();
   auto pClip = std::find_if( clips.begin(), pEnd,
...
// STRONG-GUARANTEE
{
   ...
   // Delete second clip
   auto &clips = GetData()->GetClips();
   auto it = FindClip(clips, clip2);
   clips.erase(it);
}

void WaveTrackData::Resample(int rate, ProgressDialog *progress)
// WEAK-GUARANTEE
...
{
   return FillSortedClipArray<WaveClipPointers>(GetData()->GetClips());
}

WaveClipConstPointers WaveTrack::SortedClipArray() const
{
   return FillSortedClipArray<WaveClipConstPointers>(GetData()->GetClips());
}
...
auto WaveTrackData::AllClipsIterator::operator ++ () -> AllClipsIterator &
{
...
void WaveTrackData::AllClipsIterator::push( WaveClipHolders &clips )
{
   auto pClips = &clips;
   while (!pClips->empty()) {
