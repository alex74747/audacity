...
public:
   ...
   /*! That is, it was constructed as null, or its weak reference is expired */
   ...
   void Append(
      ...
      const Item::State &state = {},
      Item::ID ItemID = Item::InvalidID )
   { Append( Item::Type::Normal, text, std::move( action ), state, ItemID ); }
   ...
   void AppendRadioItem(
      ...
      const Item::State &state = {},
      Item::ID ItemID = Item::InvalidID )
   { Append( Item::Type::Radio, text, std::move( action ), state, ItemID ); }
   ...
   void AppendCheckItem(
      ...
      const Item::State &state = {},
      Item::ID ItemID = Item::InvalidID )
   { Append( Item::Type::Check, text, std::move( action ), state, ItemID ); }
   ...
   struct Iterator : ValueIterator< Info >{
      ~Iterator();
      Iterator( Iterator && );
      Iterator &operator ++ ();
      Info operator * ();
   ...
   //! @return true of the item was found
   bool SetState(
...
public:
   ...
   //! A weak reference from a frame that a menu bar is attached to
   ...
   /*! Insertion or removal of items invalidates iterators */
   struct Iterator : ValueIterator< BarInfo >{
      ~Iterator();
      Iterator( Iterator && );
      Iterator &operator ++ ();
      BarInfo operator * ();
