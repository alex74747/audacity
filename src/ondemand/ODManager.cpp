...
void ODManager::AddTask(ODTask* task)
{
   ...
   std::lock_guard< std::mutex >
      // Sequence of lock acquisition is important!
      lock0{ mQueueNotEmptyCondLock },
      lock1{ mTasksMutex };
...
void ODManager::DispatchLoop()
{
   ...
   auto tasksInArray = [this](const Guard&){
      return !mTasks.empty();
   };
...
//    wxPrintf("ODManager thread running \n");
      ...
      // thread when ODManager is being destroyed.
      {
         ...
         mQueueNotEmptyCond.wait( locker, [&]{
            ...
            if ( paused() )
               return false;
            return tasksInArray( Guard{ mTasksMutex } );
         } );
         if ( terminated )
            return;
      }
