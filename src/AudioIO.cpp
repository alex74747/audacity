...
void AudioIO::StartMonitoring( const AudioIOStartStreamOptions &options )
{
   ...
   bool success;
   auto captureFormat = SampleTrack::SampleFormatChoice();
                         ...
                         const AudioIOStartStreamOptions &options)
{
   ...
   successAudio = successAudio &&
      std::all_of(mAudioIOExt.begin(), mAudioIOExt.end(),
         [this, &tracks, t0](auto &pExt){
            return pExt->StartOtherStream( tracks,
                          ...
                          const PaStreamCallbackFlags statusFlags, void *userData )
{
   ...
   return gAudioIO->AudioCallback(
      (constSamplePtr)inputBuffer, (float*)outputBuffer, framesPerBuffer,
   ...
   )
{
   ...
   if (inputBuffer && mSoftwarePlaythrough) {
      DoSoftwarePlaythrough(inputBuffer, mCaptureFormat,
                              numCaptureChannels,
                              outputBuffer, (int)framesPerBuffer);
   }
...
unsigned AudioIoCallback::CountSoloingTracks(){
   ...
   for(unsigned t = 0; t < numPlaybackTracks; t++ )
      if( mPlaybackTracks[t]->GetSolo() )
         numSolo++;
   numSolo += std::accumulate(mAudioIOExt.begin(), mAudioIOExt.end(), 0,
      [](unsigned sum, auto &pExt){
         return sum + pExt->CountOtherSoloTracks(); });
   return numSolo;
}
