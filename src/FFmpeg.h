...
struct streamContext
{
   ...
   unsigned             m_decodedAudioSamplesValidSiz{};   // # valid bytes in m_decodedAudioSamples
   unsigned             m_initialchannels{};               // number of channels allocated when we begin the importing. Assumes that number of channels doesn't change on the fly.

   unsigned             m_samplesize{};                    // input sample size in bytes
   AVSampleFormat       m_samplefmt{ AV_SAMPLE_FMT_NONE  }; // input sample format

   unsigned             m_osamplesize{};                   // output sample size in bytes
