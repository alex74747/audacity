...
(const wxMouseEvent &event, const AudacityProject *pProject)
{
   return HitTestResult(HitPreview(event, pProject), &Instance());
}
...
(const TrackPanelMouseEvent &evt, AudacityProject *)
{
   ...
   else if (event.m_x >= left + mRect.width - 1) {
      mZoomEnd = left + mRect.width - 1;
   }
 ...
 wxWindow *)
{
   ...
   if (IsDragZooming())
   {
      ...
      double multiplier =
         (viewInfo.PositionToTime(mRect.width - 1) - viewInfo.PositionToTime(0)) /
         (right - left);
      if (event.ShiftDown())
