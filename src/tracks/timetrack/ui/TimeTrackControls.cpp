...
private:
   virtual void InitMenu(Menu *pMenu, void *pUserData)
   {
      ...
      pMenu->Enable(OnTimeTrackLinID, pTrack->GetDisplayLog());
      pMenu->Enable(OnTimeTrackLogID, !pTrack->GetDisplayLog());
...
BEGIN_POPUP_MENU(TimeTrackMenuTable)
   ...
   POPUP_MENU_ITEM(OnTimeTrackLinID, _("&Linear"), OnTimeTrackLin)
   POPUP_MENU_ITEM(OnTimeTrackLogID, _("L&ogarithmic"), OnTimeTrackLog)
   POPUP_MENU_SEPARATOR()
