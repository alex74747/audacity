...
SampleHandle::SampleHandle()
   : mClickedTrack(NULL)
   , mClickedTrackTop(0)
   , mClickedStartSample(0)
   , mLastDragSample(0)
   , mLastDragSampleValue(0.0f)
   , mAltKey(false)
{
}
...
(const wxMouseEvent &event, const AudacityProject *pProject, bool unsafe)
{
   ...
   static std::auto_ptr<wxCursor> smoothCursor(
      new wxCursor(wxCURSOR_SPRAYCAN)
   );
   static std::auto_ptr<wxCursor> pencilCursor(
 ...
 const AudacityProject *pProject, Track *pTrack)
{
   ...
   if (WaveTrack::Waveform != displayType)
      return HitTestResult();  // Not a wave, so return.
...
(const TrackPanelMouseEvent &evt, AudacityProject *pProject)
{
   ...
   if (event.m_altDown)
   {
      ...
      std::vector<float> sampleRegion(sampleRegionSize);
      std::vector<float> newSampleRegion;
      newSampleRegion.reserve(1 + 2 * SMOOTHING_BRUSH_RADIUS);

      //Get a sample  from the track to do some tricks on.
      mClickedTrack->Get((samplePtr)&sampleRegion[0], floatSample,
         (int)mClickedStartSample - SMOOTHING_KERNEL_RADIUS - SMOOTHING_BRUSH_RADIUS,
         sampleRegionSize);
      int ii, jj;

      //Go through each point of the smoothing brush and apply a smoothing operation.
      for (jj = -SMOOTHING_BRUSH_RADIUS; jj <= SMOOTHING_BRUSH_RADIUS; ++jj) {
         float sumOfSamples = 0;
         for (ii = -SMOOTHING_KERNEL_RADIUS; ii <= SMOOTHING_KERNEL_RADIUS; ++ii) {
         ...
         newSampleRegion.push_back(
            sumOfSamples / ((SMOOTHING_KERNEL_RADIUS + 1) *(SMOOTHING_KERNEL_RADIUS + 1)));
      }
      ...
      mClickedTrack->Set((samplePtr)&newSampleRegion[0], floatSample,
...
(const TrackPanelMouseEvent &evt, AudacityProject *pProject)
{
   ...
   const int start = std::min(s0, mLastDragSample);
   const int end = std::max(s0, mLastDragSample);
 ...
 wxWindow *)
{
   ...
   mClickedTrack = NULL;       //Set this to NULL so it will catch improper drag events.
   ...
   (const wxMouseEvent &event, const ViewInfo &viewInfo, double t0)
{
   ...
   const int yy = event.m_y - mClickedTrackTop;
