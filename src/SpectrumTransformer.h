...
public:
   ...
   SpectrumTransformer(
       ...
       bool leadingPadding, /*!<
         Whether to start the queue with windows that partially overlap the first window of input samples */
       bool trailingPadding); /*!<
         Whether to stop the procedure after the last complete window of input is added to the queue */
   ~SpectrumTransformer();
   ...
   /*! More queue initializations can be done here.
      @return false to abort processing. Default implemantation just returns true. */
