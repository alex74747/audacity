...
#define FN(X) FNT(CursorAndFocusCommands, this, & CursorAndFocusCommands :: X)
...
void CursorAndFocusCommands::Create(CommandManager *c)
{
   ...
   c->AddItem(wxT("StoredCursorPosition"), _("Stored Cursor Pos&ition"), FN(OnCursorPositionStore),
      WaveTracksExistFlag,
      WaveTracksExistFlag);
...
void CursorAndFocusCommands::OnSelectCursorStoredCursor()
{
...
void CursorAndFocusCommands::OnCursorPositionStore()
{
...
void CursorAndFocusCommands::NextOrPrevFrame(bool forward)
{
   ...
   wxWindow *const begin [rotationSize] = {
      mProject->GetTopPanel(),
      mProject->GetTrackPanel(),
      mProject->GetToolManager()->GetBotDock(),
   };
...
void CursorAndFocusCommands::OnPrevTrack(bool shift)
{
   ...
   if( shift )
   {
      ...
      {
         trackPanel->SelectTrack( t, false, false );
      ...
      if( tSelected && !pSelected )
      {
         trackPanel->SelectTrack( t, true, false );
         trackPanel->SetFocusedTrack( p );   // move focus to next track down
         trackPanel->EnsureVisible( p );
         mProject->ModifyState(false);
         return;
