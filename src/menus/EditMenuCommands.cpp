...
EditMenuCommands::EditMenuCommands(AudacityProject *project)
   : mProject(project)
   , mCursorAndFocusCommands(new CursorAndFocusCommands(project))
{
}
...
void EditMenuCommands::Create(CommandManager *c)
{
   c->BeginMenu(_("&Edit"));
   {
      ...
      mCursorAndFocusCommands->Create(c);
...
void EditMenuCommands::CreateNonMenuCommands(CommandManager *c)
{
   ...
   mCursorAndFocusCommands->CreateNonMenuCommands(c);
}
...
void EditMenuCommands::OnSplitNew()
{
   ...
   for (Track *n = iter.First(); n; n = iter.Next()) {
      if (n->GetSelected()) {
         ...
         ViewInfo &viewInfo = mProject->GetViewInfo();
         if (n->GetKind() == Track::Wave) {
            ((WaveTrack*)n)->SplitCut(viewInfo.selectedRegion.t0(),
                                      viewInfo.selectedRegion.t1(), &dest);
         }
#if 0
         ...
         if (dest) {
            ...
            dest->SetLinked(n->GetLinked());
            dest->SetName(n->GetName());
            dest->SetOffset(wxMax(viewInfo.selectedRegion.t0(), offset));
            mProject->GetTracks()->Add(dest);
         }
      }
...
void EditMenuCommands::EditClipboardByLabel(EditDestFunction action)
{
   ...
   for (n = iter.First(); n; n = iter.Next())
   {
      if (n->GetKind() == Track::Wave && (allTracks || n->GetSelected()))
      {
         ...
         if (merged)
            AudacityProject::msClipboard->Add(merged);
      }
   }

   AudacityProject::msClipT0 = regions.Item(0)->start;
   AudacityProject::msClipT1 = regions.Item(regions.GetCount() - 1)->end;
...
void EditMenuCommands::ClearClipboard()
{
   ...
   if (AudacityProject::msClipboard) {
      AudacityProject::msClipboard->Clear(true);
   }
}
...
{
   mCursorAndFocusCommands->OnSelectAll();
}

void EditMenuCommands::SelectAllIfNone()
{
   mCursorAndFocusCommands->SelectAllIfNone();
}

void EditMenuCommands::OnSelectNone()
{
   mCursorAndFocusCommands->OnSelectNone();
}
