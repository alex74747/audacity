...
void LabelTrack::ComputeTextPosition(const wxRect & r, size_t index) const
{
   ...
   auto x     = labelStruct.x;  // left endpoint
   auto x1    = labelStruct.x1; // right endpoint.
   int width = labelStruct.width;

   wxInt64 xText; // This is where the text will end up.

   // Will the text all fit at this zoom?
   bool bTooWideForScreen = width > (r.width - 2 * xExtra);
// bool bSimpleCentering = !bTooWideForScreen;
   const bool bSimpleCentering = false;
   ...
   else
   {
      ...
      if( rx1 > rx0 ) // Avoid divide by zero case.
      {
         ...
         // Instead use:
         xText = xText0 +
            (wxInt64)((xText1 - xText0) * (((float)(r.x - rx0)) / (rx1 - rx0)));
      }
      else
      {
...
void LabelTrack::ComputeLayout(const wxRect & r, const ZoomInfo &zoomInfo) const
{
   ...
   {
      const auto xStart = std::numeric_limits<wxInt64>::min();
      for (auto &x : xUsed)
         x = xStart;
   }
   int nRowsUsed = 0;

   { size_t i = 0; --i; for (auto &labelStruct : mLabels) { ++i;
...
void LabelStruct::DrawLines(wxDC & dc, const wxRect & r) const
{
   ...
   const int yIconStart =
      y - (LabelTrack::mIconHeight / 2) + 1 + (LabelTrack::mTextHeight + 3) / 2;
   ...
   if( y >= 0 )
   {
      if((x >= r.x) && (x < (r.x + r.width)))
      {
         ...
         AColor::Line(dc, (wxCoord)x, r.y,  (wxCoord)x, yIconStart - 1);
         AColor::Line(dc, (wxCoord)x, yIconEnd, (wxCoord)x, r.y + r.height);
      }
      if((x1 >= r.x) && (x1 < (r.x + r.width)))
      {
         // Draw line above and below right dragging widget.
         AColor::Line(dc, (wxCoord)x1, r.y,  (wxCoord)x1, yIconStart - 1);
         AColor::Line(dc, (wxCoord)x1, yIconEnd, (wxCoord)x1, r.y + r.height);
      }
   }
   else
   {
      ...
      if((x >= r.x) && (x < (r.x + r.width)))
         AColor::Line(dc, (wxCoord)x, r.y, (wxCoord)x, r.y + r.height);
      if((x1 >= r.x) && (x1 < (r.x + r.width)))
         AColor::Line(dc, (wxCoord)x1, r.y, (wxCoord)x1, r.y + r.height);
   }
}
   ...
   (wxDC & dc, const wxRect & r, int GlyphLeft, int GlyphRight) const
{
   ...
   const int yStart =
      y - LabelTrack::mIconHeight / 2 + (LabelTrack::mTextHeight + 3) / 2;
   ...
   if((x >= r.x) && (x < (r.x + r.width)))
      dc.DrawBitmap(LabelTrack::GetGlyph(GlyphLeft),
          (wxCoord)x - xHalfWidth, yStart, true);
   ...
   if((x1 >= r.x) && (x1 < (r.x + r.width)) /*&& (x1>x+LabelTrack::mIconWidth)*/)
      dc.DrawBitmap(LabelTrack::GetGlyph(GlyphRight),
          (wxCoord)x1 - xHalfWidth, yStart, true);
}
...
void LabelStruct::DrawText(wxDC & dc, const wxRect & r) const
{
   ...
   {
      const auto xStart =
         std::max<wxInt64>(r.x, xText - LabelTrack::mIconWidth / 2);
      const auto xEnd =
         std::min<wxInt64>(r.x + r.width, xText + width + LabelTrack::mIconWidth / 2);
      const auto xWidth = xEnd - xStart;

      if( (xStart < (r.x + r.width)) && (xEnd > r.x) && (xWidth > 0))
      {
         // Now draw the text itself.
         dc.DrawText(title, (wxCoord)xText, y - LabelTrack::mTextHeight / 2);
      }
   }
...
void LabelStruct::DrawTextBox(wxDC & dc, const wxRect & r) const
{
   ...
   {
      const auto bound = r.x + r.width;
      const auto half = xBarShorten / 2;
      const int xStart =
         wxMax(r.x, BoundedPosition(x, bound, (unsigned)half) + half);
      const int xEnd =
         wxMin(bound, BoundedPosition(x1, bound, (unsigned)half) - half);
      const int xWidth = xEnd - xStart;

      if( (xStart < (r.x + r.width)) && (xEnd > r.x) && (xWidth > 0))
      {

         wxRect bar( (int)xStart, y - yBarHeight / 2 + yFrameHeight / 2,
            (int)xWidth, yBarHeight);
         if( x1 > x + xBarShorten )
   ...
   // half an icon width at each end.
   {
      ...
      {
          wxRect frame(
            (int)xStart, y - yFrameHeight / 2,
            (int)xWidth, yFrameHeight );
...
void LabelStruct::DrawHighlight
   ( wxDC & dc, wxInt64 xP1, wxInt64 xP2, int charHeight) const
{
   ...
   if (xPos1 < xPos2)
      ...
      dc.DrawRectangle((wxCoord)xPos2 - 1, y - charHeight/2,
                       (wxCoord)xPos1 - xPos2 + 1, charHeight);
}

void LabelStruct::getXPos( wxDC & dc, wxInt64 * xPos1, size_t cursorPos) const
{
   *xPos1 = xText;
   if( cursorPos > 0 )
   {
...
bool LabelTrack::CalcCursorX(wxInt64 * x) const
{
   if (mSelIndex >= 0) {
      wxMemoryDC dc;
      ...
      mLabels[(size_t)mSelIndex].getXPos(dc, x, mCurrentCursorPos);
...
void LabelTrack::CalcHighlightXs(wxInt64 *x1, wxInt64 *x2) const
{
   wxMemoryDC dc;
   ...
   auto pos1 = mInitialCursorPos, pos2 = mCurrentCursorPos;
   if (pos1 > pos2)
      std::swap(pos1, pos2);

   wxASSERT(mSelIndex >= 0);
   const auto &labelStruct = mLabels[(size_t)mSelIndex];
   ...
   const ZoomInfo &zoomInfo) const
{
   ...
   if ((mInitialCursorPos != mCurrentCursorPos) && (mSelIndex >= 0))
   {
      ...
      mLabels[(size_t)mSelIndex].DrawHighlight(dc, xpos1, xpos2, mFontHeight);
   }
   ...
   if( mDrawCursor && mSelIndex >= 0 )
   {
      ...
      xPos = BoundedPosition(xPos, dc.GetSize().GetWidth(), 1);

      wxPen currentPen = dc.GetPen();
      const int CursorWidth = 2;
      currentPen.SetWidth(CursorWidth);
      AColor::Line(dc,
                   (wxCoord)xPos - 1, labelStruct.y - mFontHeight / 2 + 1,
                   (wxCoord)xPos - 1, labelStruct.y + mFontHeight / 2 - 1);
...
size_t LabelTrack::FindCurrentCursorPosition(int xPos)
{
   size_t result = 0;
   wxMemoryDC dc;
   if(msFont.Ok())
   ...
   while (!finished && (charIndex < length + 1))
   {
      ...
      auto bound = labelStruct.xText + partWidth - oneWidth * 0.5;
...
bool LabelTrack::CutSelectedText()
{
   ...
   wxString left, right;
   auto &labelStruct = mLabels[(size_t)mSelIndex];
   auto &text = labelStruct.title;

   auto init = mInitialCursorPos;
   auto cur = mCurrentCursorPos;
   if (init > cur)
      std::swap(init, cur);
...
bool LabelTrack::PasteSelectedText(double sel0, double sel1)
{
   ...
   auto cur = mCurrentCursorPos, init = mInitialCursorPos;
   if (init > cur)
      std::swap(init, cur);
...
int LabelTrack::OverGlyph(int x, int y)
{
   ...
   { int i = -1; for (auto &labelStruct : mLabels) { ++i;
      ...
      if( abs(labelStruct.y - (y - (LabelTrack::mTextHeight + 3) / 2)) < d1 &&
               std::abs(labelStruct.x1 - d2 - x) < d1)
      {
         mMouseOverLabelRight = i;
         if(std::abs(labelStruct.x1 - x) < d2 )
         {
            mbHitCenter = true;
            ...
            if(std::abs(labelStruct.x1 - labelStruct.x) < 1.0 )
            {
               result |=1;
      ...
      // of the same label.
            ...
            std::abs(labelStruct.x + d2 - x) < d1 )
      {
         mMouseOverLabelLeft = i;
         if(std::abs(labelStruct.x - x) < d2 )
            mbHitCenter = true;
         result |= 1;
      }
...
void LabelTrack::MayAdjustLabel( int iLabel, int iEdge, bool bAllowSwapping, double fNewTime)
{
   if( iLabel < 0 )
      return;
   LabelStruct &labelStruct = mLabels[ (size_t)iLabel ];
...
void LabelTrack::MayMoveLabel( int iLabel, int iEdge, double fNewTime)
{
   if( iLabel < 0 )
      return;
   mLabels[ (size_t)iLabel ].MoveLabel( iEdge, fNewTime );
}

// Constrain function, as in processing/arduino.
// returned value will be between min and max (inclusive).
static int Constrain( wxInt64 value, int min, int max )
{
   wxASSERT( min <= max );
   auto result = value;
   if( result < min )
   ...
   SelectedRegion *newSel)
{
   if(evt.LeftUp())
   {
      ...
      if(mMouseOverLabelLeft >= 0) {
         auto &labelStruct = mLabels[(size_t)mMouseOverLabelLeft];
         lupd = labelStruct.updated;
         labelStruct.updated = false;
      }
      if(mMouseOverLabelRight >= 0) {
         auto &labelStruct = mLabels[(size_t)mMouseOverLabelRight];
   ...
   if(evt.Dragging())
   {
      ...
      if( mSelIndex >=0 )
      {
         ...
         *newSel = mLabels[(size_t)mSelIndex].selectedRegion;
      }
      SortLabels();
   }
...
void LabelTrack::HandleTextDragRelease(const wxMouseEvent & evt)
{
   ...
   if (evt.RightUp()) {
      if ((mSelIndex >= 0) &&
          OverTextBox(GetLabel((size_t)mSelIndex), evt.m_x, evt.m_y)) {
   ...
   SelectedRegion *newSel)
{
   if (evt.ButtonDown())
   {
      ...
      if (mIsAdjustingLabel)
      {
         ...
         {
            t = (mLabels[(size_t)mMouseOverLabelRight].getT1() +
                 mLabels[(size_t)mMouseOverLabelLeft].getT0()) / 2.0f;
         ...
         {
            t = mLabels[(size_t)mMouseOverLabelLeft].getT0();
         }
         ...
         *newSel = labelStruct.selectedRegion;
         ...
         if (evt.LeftDown()) {
            ...
            if (evt.ShiftDown()) {
               ...
               if (abs((int)position - (int)mCurrentCursorPos) >
                   abs((int)position - (int)mInitialCursorPos))
...
bool LabelTrack::OnKeyDown(SelectedRegion &newSel, wxKeyEvent & event)
{
   ...
   if (mSelIndex >= 0) {
      ...
      case WXK_NUMPAD_RIGHT:
         // Moving cursor right
         if (mCurrentCursorPos < title.length()) {
            mCurrentCursorPos++;
            if (mods == wxMOD_SHIFT)
               ;
            else
      ...
      case WXK_NUMPAD_TAB:
         ...
         {
            LabelStruct &newLabel = mLabels[(size_t)mSelIndex];
   ...
   else
   {
      ...
      case WXK_NUMPAD_TAB:
         if (!mLabels.empty()) {
            auto len = mLabels.size();
            if (event.ShiftDown()) {
               mSelIndex = (int)len - 1;
               if (newSel.t0() > mLabels[0].getT0()) {
                  while (mSelIndex >= 0 &&
                         mLabels[(size_t)mSelIndex].getT0() > newSel.t0()) {
                     mSelIndex--;
                  }
               }
            } else {
            ...
            if (mSelIndex >= 0 && mSelIndex < (int)len) {
               const auto &labelStruct = mLabels[(size_t)mSelIndex];
...
void LabelTrack::ShowContextMenu()
{
...

   {
      ...
      wxASSERT(mSelIndex >= 0);
      const LabelStruct *ls = GetLabel((size_t)mSelIndex);
      ...
      parent->PopupMenu(&menu,
                        BoundedPosition(x, parent->GetSize().GetWidth(), 0),
                        ls->y + (mIconHeight / 2) - 1);
   }
}
...
bool LabelTrack::PasteOver(double t, const Track * src)
{
   ...
   for (auto &labelStruct: sl->mLabels) {
      ...
      mLabels.insert(mLabels.begin() + (int)pos++, l);
      len++;
   }
...
bool LabelTrack::Silence(double t0, double t1)
{
   ...
   for (size_t i = 0; i < len; ++i) {
      ...
      if (relation == LabelStruct::WITHIN_LABEL)
      {
         ...
         ++i;
         mLabels.insert(mLabels.begin() + (int)i, l);
      }
      else if (relation == LabelStruct::ENDS_IN_LABEL)
      {
...
size_t LabelTrack::GetNumLabels() const
{
   return mLabels.size();
}

const LabelStruct *LabelTrack::GetLabel(size_t index) const
{
...
size_t LabelTrack::AddLabel(const SelectedRegion &selectedRegion,
...
void LabelTrack::DeleteLabel(size_t index)
{
   wxASSERT(index < mLabels.size());
   mLabels.erase(mLabels.begin() + (int)index);
   // IF we've deleted the selected label
   // THEN set no label selected.
   if( mSelIndex == (int)index )
   {
      mSelIndex = -1;
      mCurrentCursorPos = 1;
   }
...
void LabelTrack::SortLabels()
{
   ...
   while (true)
   {
      ...
      while (i < nn &&
             mLabels[(size_t)i - 1].getT0() <= mLabels[(size_t)i].getT0())
         ++i;
      if (i >= nn)
         break;
      ...
      int j = i - 2;
      while( (j >= 0) &&
            (mLabels[(size_t)j].getT0() > mLabels[(size_t)i].getT0()) )
         --j;
      ++j;
