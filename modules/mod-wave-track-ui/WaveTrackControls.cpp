...
PopupMenuTableEntry::StateFunction stateFn( Table &handler,
   const ValueFinder &findValue, int value )
{
   return [&handler, findValue, value]()
...
struct RateMenuTable : PopupMenuTable
{
   ...
   static int IndexOfRate(int rate);
...
BEGIN_POPUP_MENU(RateMenuTable)
   static const auto fn = [this](int rate){
      return stateFn( *this,
         []( WaveTrack &track ){
            return IndexOfRate( (int)track.GetRate() ); },
         rate );
   };
   ...
   for ( auto rate : gRates ) {
      AppendCheckItem(
         wxString::Format( "%d", rate ), OnRate8ID + ii, XXO("%d Hz").Format( rate ),
         POPUP_MENU_FN( OnRateChange ), fn( ii ) );
      ++ii;
   }

   AppendCheckItem( "Other", OnRateOtherID, XXO("&Other..."),
      POPUP_MENU_FN( OnRateOther ), fn(-1) );

END_POPUP_MENU()

/// Converts a sampling rate to a wxWidgets menu item id
int RateMenuTable::IndexOfRate(int index)
{
   ...
   BeginSection( "SubViews" );
      ...
      for ( const auto &type : AllTypes() ) {
         static const auto stateFn = [this]( bool radio, int index ){ return
            [this, radio, index]() -> Widgets::MenuItemState {
