...
namespace {
void SetLastdBRange(
   const WaveformSettingsCache &cache, const WaveTrack &track)
{
   ...
   const TrackPanelMouseEvent &evt, AudacityProject *pProject, WaveTrack *wt)
{
   ...
   if (isDB && event.ShiftDown() && event.CmdDown()) {
      ...
      if (fixedMagnification) {
         ...
         for (auto channel : TrackList::Channels(wt)) {
            auto &cache = WaveformSettingsCache::Get(*channel);
      ...
      const int height = evt.rect.GetHeight();
      {
         ...
         for (auto channel : TrackList::Channels(wt))
            WaveformSettingsCache::Get(*channel)
   ...
   const wxRect &rect, const WaveTrack *wt )
{
   ...
   WaveformSettings::ScaleType scaleType = settings.scaleType;
