   ...
   bool fixedMousePoint)
{
   ...
   {
      SpectrogramSettingsCache::Get(*pTrack).GetBounds(*pTrack, min, max);
   ...
   for (auto channel : TrackList::Channels(pTrack))
      SpectrogramSettingsCache::Get(*channel).SetBounds(min, max);
      ...
      for (int ii = 0, nn = names.size(); ii < nn; ++ii) {
         AppendRadioItem( names[ii].Internal(),
            ...
            [this, ii]() -> Widgets::MenuItemState {
               ...
               return { true,
                  ii == (int)(SpectrogramSettings::Get(*wt).scaleType ) };
            }
         );
      }
   }
