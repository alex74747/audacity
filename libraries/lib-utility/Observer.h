...
/*!
 This has a virtual destructor so that it can be the operand of dynamic_cast,
 but that is not a requirement for Publisher's template argument.
 */
struct UTILITY_API Message {
public:
   ~Message();
};
...
struct RecordBase : RecordLink {
   explicit RecordBase(RecordList &list) : list{ list } {}
...
struct UTILITY_API RecordList
   ...
   using Visitor =
      std::function< bool(const RecordBase &record, const void *arg) >;
   //! @pre `visitor != nullptr`
   explicit RecordList(ExceptionPolicy *pPolicy, Visitor visitor);
   ~RecordList() noexcept;
   //! @pre `pRecord != nullptr`
   void PushFront(std::shared_ptr<RecordBase> &&pRecord);
   bool Visit(const void *arg);
private:
   friend Subscription;
   void Remove(std::shared_ptr<RecordBase> pRecord) noexcept;
...
private:
   template<typename T, bool B> friend class Publisher;
...
public:
   ...
   /*! Later-connected are called earlier; if !NotifyAll, any callback may
    ...
    @return if `NotifyAll`, then void; else whether any callback returned true
    */
   CallbackReturn Publish(const Message &message);

public:
   struct Record : detail::RecordBase {
      explicit Record(detail::RecordList &list, Callback callback)
         : detail::RecordBase{ list }
...
: m_list{ std::allocate_shared<detail::RecordList>( a, pPolicy,
    [pPolicy]( // the visitor
      ...
      try {
         ...
         if (pPolicy)
            return pPolicy->OnEachFailedCallback();
         return false;
      }
   }
) }
, m_factory( [list = m_list.get(), a = move(a)](Callback callback) {
   return std::allocate_shared<Record>(a, *list, move(callback));
} )
{}
...
auto Publisher<Message, NotifyAll>::Subscribe(Callback callback)
   -> Subscription
{
   auto pRecord = m_factory(move(callback));
   auto result = Subscription{ pRecord };
   m_list->PushFront(move(pRecord));
   return result;
}
